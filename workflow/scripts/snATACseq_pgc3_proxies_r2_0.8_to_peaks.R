...
snps <- read_excel("Desktop/single_nuclei/snRNAseq/tests/PGC3_SZ_index_SNPs.xlsx") %>%
...
snps_in_LD <- LDlinkR::LDproxy_batch(snp = snps, token = "5d1a2b50035c")
...
for (SNP in snps$`top-index`) {
  ...
  tryCatch(
    ...
    proxies <<- read_delim(paste0("~/Desktop/single_nuclei/snRNAseq/tests/PGC3_proxy_SNPs/", SNP, ".txt"), 
  ...
  snps_in_proxies_r2_0.8 <- nrow(proxies)
...
snp_proxies_unique_df <- snp_proxies_df %>% distinct(R2, .keep_all = TRUE)
