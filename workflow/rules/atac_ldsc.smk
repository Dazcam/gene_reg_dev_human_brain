...
rule all:
    input:
        expand(RESULTS_DIR + "snATACseq_LDSC_summary_SCZ.tsv"),
        expand(RESULTS_DIR + "snATACseq_LDSC_report_SCZ.html") 
...
rule partitioned_heritability:
    input:   GWAS = GWAS_DIR + "SCZ_hg19_ldsc_ready.sumstats.gz",
    ...
    output:  PART_HERIT_DIR + "snATACseq_LDSC_{CELL_TYPE}_SCZ.results"
    ...
    params:  weights = REF_DIR + "weights_hm3_no_hla/weights.",
             ...
             out_file = PART_HERIT_DIR + "snATACseq_LDSC_{CELL_TYPE}_SCZ"
...
rule create_partHerit_summary:
    ...
    input:   expand(PART_HERIT_DIR + "snATACseq_LDSC_{CELL_TYPE}_SCZ.results", CELL_TYPE = config["ATAC_CELL_TYPES"])
    output:  RESULTS_DIR + "snATACseq_LDSC_summary_SCZ.tsv"
    ...
    shell:
             ...
             head -1 {params.ph_dir}snATACseq_LDSC_InN.1.500bpExt.FDR.0.01.ge_SCZ.results > {output}

             File={params.cell_types}
             Lines=$(cat $File)
             for Line in $Lines
             do
             grep L2_1 {params.ph_dir}snATACseq_LDSC_"$Line"_SCZ.results | sed "s/L2_1/$Line/g" >> {output}
             done
...
rule ldsc_mrkdwn_report:
    input:   summary = RESULTS_DIR + "snATACseq_LDSC_summary_SCZ.tsv",
             markdown = MARKDOWN_DIR + "snATACseq_LDSC_report_SCZ.Rmd"
    output:  RESULTS_DIR + "snATACseq_LDSC_report_SCZ.html"
