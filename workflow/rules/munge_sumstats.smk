...
rule sumstats_for_ldsc_hg19:
    input:   snps = SCRATCH + "ldsc/reference_files/w_hm3.snplist",
             gwas = GWAS_DIR + "GWAS_sumstats_standardised/{GWAS}_hg19_withZv2_sumstats.tsv"
    output:  LDSC_GWAS_DIR + "{GWAS}_hg19_ldsc_ready.sumstats.gz"
    conda:   "envs/ldsc.yml"
    message: "Munging sumstats for {input.gwas} for ldsc compatibility"
    params:  out = LDSC_GWAS_DIR + "{GWAS}_hg19_ldsc_ready"
    log:     "logs/munge_sumstats/{GWAS}_hg19_sumstats_for_ldsc.log"
    shell:
        """
